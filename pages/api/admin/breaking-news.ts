...
export default async function handler(
    ...
    const enforceActiveCap = async (protectId: string): Promise<string[]> => {
        ...
        const activeItems: { _id: string }[] = await adminSanityClient.fetch(
            `*[_type == "breakingNews" && active == true && !(_id in path("drafts.**"))] | order(priority asc, _createdAt desc) { _id }`
        );
    ...
    try {
        ...
        switch (method) {
            case 'GET':
                ...
                const query = `*[_type == "breakingNews"${filter}] | order(priority asc, _createdAt desc) {
                    _id,
                    title,
                    link,
                    active,
